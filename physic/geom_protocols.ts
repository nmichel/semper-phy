...
import { Aligner, CircleCollider, Cloner, Collider, PointCaster, PolygonCollider, RayCaster, Render, Transformer } from './protocols/protocols';
...
defimpl(Transformer, Polygon, {
  toWorld: (polygon: Polygon, frame: Frame): Polygon => {
    return new Polygon(polygon.vertices.map(v => frame.positionToWorld(v)), polygon.radius);
  },
...
defimpl(CircleCollider, Polygon, {
  overlap: (polygon: Polygon, circle: Circle): {depth: number, normal: Vector2} | null => {
    const { minVect } = polygon.vertices.reduce((acc, vertex: Vector2) => {
      const { minDist } = acc
